...
PartOfSpeech_t
MobyToPartOfSpeech( char ch )
{
    switch( ch ) {
        case 'N': return POS_Noun;
        case 'p': return POS_Plural;
        case 'h': return POS_NounPhrase;
        case 'V': return POS_VerbParticiple;
        case 't': return POS_VerbTransitive;
        case 'i': return POS_VerbIntrasitive;
        ...
        case 'D': return POS_DefiniteArticle;
        case 'I': return POS_IndefiniteArticle;
...
bool
WordDatabase::load( const char* filename )
{
    // open file.
    char linebuff[100];
    ...
    for( ;; ) {
        ...
        unsigned numPron = generateSyllables( pron, pronBuffer,
                sizeof(pronBuffer)/sizeof(*pronBuffer) - 1, &numSyllables );
        if ( numPron == 0 ) {
            continue;
        }
...
void
WordDatabase::addSynonym( Word* base, Word* synonym )
{
    //printf("%s->%s\n", base->text, synonym->text);
...
Word::print()
{
    printf("%s %d %s ",
            ...
            partOfSpeech == POS_VerbIntrasitive ? "Verb-Intransitive" :
...
void
Word::pronToText( unsigned short pr, char* buffer )
{
        ...
        pnumSyllables)
{
    ...
    struct StateMachine_t {
        ...
        { start, '&', a_in_dab, start },
        { start, '(', -1, got_lparen },
        { start, '[', -1, got_lparen },
        { start, '@', a_in_air, start },
        { got_lparen, '@', -1, got_lparen_at },
        { got_lparen_at, ')',  a_in_air, start },
        { got_lparen_at, ']',  a_in_air, start },
        { start, '-', ir_in_tire, start },
        { start, '@', -1, got_at },
        { got_at, 'r', u_in_burn, start },
        { got_at, '?', a_in_ado, start },
        { start, 'A', -1, got_A },
        { got_A, 'U', ow_in_how, start },
        { got_A, '?', a_in_far, start },
        { start, 'D', th_in_the, start },
        { start, 'E', e_in_red, start },
        { start, 'I', i_in_hid, start },
        { start, 'N', ng_in_bang, start },
        { start, 'O', -1, got_O },
        { got_O, 'i', oi_in_oil, start },
        { got_O, '?', o_in_dog, start },
        { start, 'R', r_in_der, start },
        { start, 'S', sh_in_she, start },
        { start, 'T', th_in_bath, start },
        { start, 'U', oo_in_book, start },
        ...
        { start, 'd', -1, got_d  },
        { got_d, 'Z', g_in_vegetably, start },
        { got_d, '?', d_in_pod, start },
        { start, 'a', -1, got_a  },
        { got_a, 'I', i_in_ice, start },
        { got_a, '?', a_in_dab, start }, // due to errors in mobydict.
        { start, 'e', -1, got_e },
        { got_e, 'I', a_in_day, start },
        { start, 'f', f_in_elf, start },
        { start, 'g', g_in_fig, start },
        { start, 'h', -1, got_h },
        { got_h, 'w', w_in_white, start },
        { got_h, '?', h_in_had, start },
        { start, 'i', e_in_see, start },
        { start, 'j', y_in_you, start },
        { start, 'k', c_in_act, start },
        { start, 'l', l_in_ail, start },
        { start, 'm', m_in_aim, start },
        { start, 'n', n_in_and, start },
        { start, 'o', -1, got_o },
        { got_o, 'U', o_in_boat, start },
        { start, 'p', p_in_imp, start },
        { start, 'r', r_in_ire, start },
        { start, 's', s_in_sip, start },
        { start, 't', -1, got_t },
        { got_t, 'S', ch_in_ouch, start },
        { got_t, '?', t_in_tap, start },
        { start, 'u', oo_in_too, start },
        { start, 'v', v_in_average, start },
        { start, 'w', w_in_win, start },
        { start, 'x', ch_in_bach, start },
        { start, 'y', eu_in_bleu, start },
        { start, 'z', z_in_zoo, start },
        { start, '/', -1, start },
        { start, '_', -1, start },
        { start, '\'', -2, start },
        { start, ',', -3, start },
...
unsigned
getCommonSuffixLength( Word* word1, Word* word2 )
{
    unsigned pos1 = 0;
    unsigned pos2 = 0;
    unsigned count = 0;
...
unsigned
MinSuffixContainingSyllable( Word* word )
{
    unsigned pos = 0;
...
void
WordDatabase::filter( WordArray& results, WordFilter* filter )
{
...
WordDatabase::makeWords( WordArray& results, const char* text )
{
    ...
    while( token ) {
        ...
        if ( word ) {
            ...
            printf("    Couldn't find word for %s\n", token);
            failedWords++;
        }
...
unsigned
WordDatabase::getSynonyms( WordArray& results, const char* wordtext )
{
...
void
WordAssembler::addSolution( WordArray& phrase )
{
    Solution solution;
    ...
    if ( (solutions.size() % 10000) == 0 ) {
        printf("Found %d solutions....\r", solutions.size() );
    }
...
void
WordAssembler::run( PartOfSpeech_t POS[], unsigned length, unsigned numSyllables,
